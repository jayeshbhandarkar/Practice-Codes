...
police department from the vehicles violating the rules. Note : For violating the rule, vehicles would be fined as 
X Rs.
...
Explanation:
Date D=3 means only odd number vehicles with are allowed. Find will be collected from 2,6 and 8 with an amount of 
300 each. Hence, the output = 900 
...
The output should be a positive integer number (Check the output in Example 1, Example e) if no fine is collected 
then print ”0”.
"""

# Accept input values
N = int(input())  # Number of vehicles
a = [int(input()) for _ in range(N)]  # List of last digits of vehicle registration numbers
D = int(input())  # Date
X = int(input())  # Fine per violating vehicle

# Determine the rule for the given date
if D % 2 == 0:
    # If the date is even, only even last digits are allowed
    violating_vehicles = [num for num in a if num % 2 != 0]
else:
    # If the date is odd, only odd last digits are allowed
    violating_vehicles = [num for num in a if num % 2 == 0]

# Calculate total fine
total_fine = len(violating_vehicles) * X
